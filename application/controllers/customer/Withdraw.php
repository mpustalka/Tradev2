...
class Withdraw extends CI_Controller 
{
    ...
    public function store()
    {
        ...
        $this->form_validation->set_rules('varify_media', 'OTP Send To', 'required|xss_clean'); 
        $this->form_validation->set_rules('walletid', 'Wallet id', 'required|xss_clean'); 
        ...
        if($this->form_validation->run()){
            ...
            #----------------------------
            // check balance 
            $blance = $this->check_balance($this->input->post('amount',TRUE));
            #----------------------------
            if($blance!=true){
                ...
                if($code_send != NULL ){
                    ...
                    // get withdraw fees
                    $fees = $this->fees_load($this->input->post('amount',TRUE),$this->input->post('method',TRUE),'withdraw');
                    #-----------------------
                    $withdraw = array(
                        'user_id  ' => $this->session->userdata('user_id'),
                        'amount' => $this->input->post('amount',TRUE),
                    ...
                    $this->session->set_flashdata('exception', "Missing Your Verify Media, Please Try Again.");
                    redirect('customer/withdraw');
                }   
            }     
    ...
    */
    public function withdraw_verify()
    {
        ...
        $id = $this->input->post('confirm_id',TRUE);
        ...
        if($data!=NULL) {
            ...
            if(!empty($smsPermission) && $set->smsPermission == 1){
            ...
            $this->db->set('status',0)
                    ->where('id',$this->input->post('confirm_id',TRUE))
                    ->where('session_id',$this->session->userdata('isLogIn'))
                    ->update('verify_tbl');
