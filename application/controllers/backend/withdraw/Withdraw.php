...
class Withdraw extends CI_Controller {
    ...
    public function userwise_withdraw_list()
    {  
        $data['title'] = display('withdraw_list');
        ...
        #
        $config["base_url"] = base_url('backend/withdraw/withdraw/withdraw_list');
        $config["total_rows"] = $this->db->where('user_id',$user_id)->from("withdraw")->count_all_results();
        //$config["total_rows"] = $this->db->count_all("withdraw")->where('user_id',$user_id);
        $config["per_page"] = 10;
        $config["uri_segment"] = 5;
        $config["last_link"] = "Last"; 
        $config["first_link"] = "First"; 
        ...
        $this->pagination->initialize($config);
        $page = ($this->uri->segment(5)) ? $this->uri->segment(5) : 0;
        $data['withdraw'] = $this->withdraw_model->user_wise($config["per_page"], $page, array('user_id'=>$user_id));
