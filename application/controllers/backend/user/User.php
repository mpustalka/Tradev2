...
class User extends CI_Controller {
    ...
    */
    ...
    { 
    	...
    	/*-----------------------------------*/
    	$this->form_validation->set_rules('username', display('username'),'required|max_length[20]|xss_clean');
    	$this->form_validation->set_rules('sponsor_id', display('sponsor_id'),'required|max_length[6]|xss_clean');
		...
		#------------------------#
    	if (!empty($uid)) {   
    		$this->form_validation->set_rules('username', display("username"), "required|max_length[100]|callback_username_check[$uid]|trim|xss_clean"); 
    	} else {
    		$this->form_validation->set_rules('username', display('username'),'required|is_unique[user_registration.username]|max_length[20]|xss_clean');
    	} 
		#------------------------#
    	if (!empty($uid)) {   
    		$this->form_validation->set_rules('email', 'Email Address', "required|valid_email|max_length[100]|callback_email_check[$uid]|trim|xss_clean"); 
    	} else {
    		$this->form_validation->set_rules('email', display('email'),'required|valid_email|is_unique[user_registration.email]|max_length[100]|xss_clean');
    	}


		#------------------------#
    	$this->form_validation->set_rules('mobile', display('mobile'),'max_length[30]|xss_clean');
    	...
    	{ 
    		...
    		$uid_query = $this->db->select('password')->where('uid', $uid)->get('user_registration')->row();
    		if(!empty($this->input->post('password',TRUE))){
    			$userpassword = md5($this->input->post('password',TRUE));
    		} else {
    			$userpassword = $uid_query->password;
    		}
    		$data['user'] = (object)$userdata = array(
    			'uid' 		  => $this->input->post('uid',TRUE),
