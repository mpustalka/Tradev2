...
class Exchange extends CI_Controller {
	...
	{ 
		...
		$coin = $this->exchange_model->findCurrency($this->input->post('cid',TRUE));
		if (!empty($coin)) {
			$coin_name = $coin->name;
		}
		else{
		...
		$this->form_validation->set_rules('wallet_data', display('wallet_data'),'required|xss_clean');
		$this->form_validation->set_rules('sell_adjustment', display('sell_adjustment'),'required|xss_clean');
		$this->form_validation->set_rules('buy_adjustment', display('buy_adjustment'),'required|xss_clean');
		$this->form_validation->set_rules('status', display('status'),'required|xss_clean');
		
		/*-----------------------------------*/ 
		$data['exchange']   = (object)$userdata = array(
			'ext_exchange_id'			=> $this->input->post('ext_exchange_id',TRUE),
			'coin_id'   				=> $this->input->post('cid',TRUE),
			'coin_name' 				=> $coin_name, 
			'wallet_data'  				=> $this->input->post('wallet_data',TRUE), 
			'sell_adjustment' 			=> $this->input->post('sell_adjustment',TRUE),
			'buy_adjustment' 			=> $this->input->post('buy_adjustment',TRUE),
	...
	{  
		$status = $this->input->post('status',TRUE);
		$receving_status = $this->input->post('receving_status',TRUE);
		$receving_status_confirm = $this->input->post('receving_status_confirm',TRUE);
		$payment_status = $this->input->post('payment_status',TRUE);
		$payment_status_confirm = $this->input->post('payment_status_confirm',TRUE);
