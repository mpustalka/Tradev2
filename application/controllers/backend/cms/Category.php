...
class Category extends CI_Controller {
	...
	{ 
		...
		if (!empty($cat_id)) {   
       		$this->form_validation->set_rules('cat_name_en', display("cat_name_en"), "required|max_length[255]|callback_slug_check[$cat_id]|xss_clean"); 

		} else {
			$this->form_validation->set_rules('cat_name_en', display('cat_name_en'),'required|is_unique[web_category.cat_name_en]|max_length[255]|xss_clean');

		}

		$slug = url_title($this->input->post('cat_name_en',TRUE), 'dash', TRUE);
		...
		$data['category']   = (object)$userdata = array(
			...
			'slug'      	=> $slug,
			'cat_name_en'   => $this->input->post('cat_name_en',TRUE),
			'cat_name_fr' 	=> $this->input->post('cat_name_fr',TRUE), 
			'parent_id'  	=> $this->input->post('parent_id',TRUE), 
			'cat_image' 	=> (!empty($image)?$image:$this->input->post('cat_image_old',TRUE)),
			'cat_title1_en' => $this->input->post('cat_title1_en',TRUE),
			'cat_title1_fr' => $this->input->post('cat_title1_fr',TRUE),
			'cat_title2_en' => $this->input->post('cat_title2_en',TRUE),
			'cat_title2_fr' => $this->input->post('cat_title2_fr',TRUE),
