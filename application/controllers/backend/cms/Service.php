...
class Service extends CI_Controller {
	...
	public function form($article_id = null)
	{
		...
		if (!empty($article_id)) {   
       		$this->form_validation->set_rules('headline_en', display("headline_en"), "required|max_length[255]|callback_slug_check[$article_id]|xss_clean"); 

		} else {
			$this->form_validation->set_rules('headline_en', display('headline_en'),'required|is_unique[web_article.headline_en]|max_length[255]|xss_clean');
			
		}

		$slug = url_title(strip_tags($this->input->post('headline_en',TRUE)), 'dash', TRUE);
