...
class Package extends CI_Controller {
	...
	{ 
		...
		/*-----------------------------------*/
		if (!empty($package_id)) {   
       		$this->form_validation->set_rules('package_name', display("package_name"), "required|max_length[250]|callback_package_check[$package_id]|xss_clean"); 
		} else {
			$this->form_validation->set_rules('package_name', display('package_name'),'required|is_unique[package.package_name]|max_length[250]|xss_clean');
		}  
		#------------------------#
		$this->form_validation->set_rules('package_details', display('package_details'),'max_length[1000]|xss_clean');
		$this->form_validation->set_rules('package_amount', display('package_amount'),'required|max_length[11]|xss_clean');
		$this->form_validation->set_rules('daily_roi', display('daily_roi'),'required|max_length[11]|xss_clean');
		...
		$this->form_validation->set_rules('period', display('period'),'required|xss_clean');
		...
		$data['package'] = (object)$userdata = array(
			...
			'package_deatils' => $this->input->post('package_deatils',TRUE), 
			'package_amount'  => $this->input->post('package_amount',TRUE), 
			'daily_roi' 	  => $this->input->post('daily_roi',TRUE),
