...
class Credit extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array(
            'backend/dashboard/message_model','customer/Transfer_model' 
		));  
    ...
    public function send_credit()
    {
        ...
        $this->form_validation->set_rules('user_id', display('user_id') ,'required|xss_clean');
        ...
        if ($this->form_validation->run()) { 
            ...
            $deposit_data = array(
                ...
                'fees'              => 0.0,
                'comments'          => $this->input->post('note',TRUE),
                'deposit_date'      => date('Y-m-d h:i:s'),
                'deposit_ip'        => $this->input->ip_address(),
                'status'            => 1
            );

            $userCheck = $this->Transfer_model->findById('user_registration', array('user_id'=>$this->input->post('user_id',TRUE)));
            ...
            if(!empty($userCheck)){
                ...
                $this->session->set_flashdata('exception','User Id Is Not Valid.');
                redirect('backend/dashboard/credit/add_credit');
            }
